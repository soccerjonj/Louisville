...
if [ ! -f "$SCRIPT_DIR/pmtiles" ]; then
    ...
    if [ -f "$SCRIPT_DIR/pmtiles" ]; then
        ...
        echo "[Sacramento Mod] pmtiles downloaded and executable."
    else
        echo "[Sacramento Mod] Error: Failed to download pmtiles. Please download it manually."
        exit 1
    fi
fi
